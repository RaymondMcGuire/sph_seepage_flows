...
namespace KIRI
{
    static __global__ void ComputePressure_CUDA(
        float *density,
        float *pressure,
        const size_t num,
        const float rho0,
        ...
        LaplacianFunc nablaW2)
    {
        ...
        {
            if (i != j)
                *a += mass[j] * ((vel[j] - vel[i]) / fmaxf(KIRI_EPSILON, density[j])) * nablaW2(length(pos[i] - pos[j]));
            ++j;
        }
        return;
    }
        ...
        GradientFunc nablaW)
    {
        while (j < cellEnd)
        {
            if (i != j)
            {
                ...
                if (dot_dvdp < 0.f)
                {
                    float pij = -nu / fmaxf(KIRI_EPSILON, (density[i] + density[j])) * (dot_dvdp / (lengthSquared(dpij) + KIRI_EPSILON));
...
#pragma unroll
        for (int m = 0; m < 27; ++m)
        {
            ...
            ComputeFluidDensity(&density[i], i, pos, mass, cellStart[hash_idx], cellStart[hash_idx + 1], W);
...
#pragma unroll
        for (int m = 0; m < 27; ++m)
        {
            ...
            ViscosityMuller2003(&a, i, pos, vel, mass, density, cellStart[hash_idx], cellStart[hash_idx + 1], nablaW2);
            ComputeBoundaryViscosity(&a, pos[i], bPos, vel[i], density[i], bVolume, bnu, rho0, bCellStart[hash_idx], bCellStart[hash_idx + 1], nablaW);
        }
...
#pragma unroll
        for (int m = 0; m < 27; ++m)
        {
            ...
            ArtificialViscosity(&a, i, pos, vel, mass, density, nu, cellStart[hash_idx], cellStart[hash_idx + 1], nablaW);
            ComputeBoundaryViscosity(&a, pos[i], bPos, vel[i], density[i], bVolume, bnu, rho0, bCellStart[hash_idx], bCellStart[hash_idx + 1], nablaW);
        }
