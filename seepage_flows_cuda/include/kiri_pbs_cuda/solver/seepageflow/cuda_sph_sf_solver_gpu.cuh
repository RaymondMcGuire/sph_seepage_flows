...
namespace KIRI
{
    ...
    __device__ void ComputeSFSandVolume(
        float *solidV,
        const size_t i,
        size_t *label,
        float3 *pos,
    ...
    template <typename Func>
    __device__ void ComputeSFAvgFlow(
        float3 *avgVelS,
        float3 *avgVelW,
        float *vS,
        float *vW,
        ...
        AdhesionFunc A)
    {
        while (j < cellEnd)
        {
            ...
            if (label[i] == 0 && label[j] == 1)
            {
                ...
                float wij = W(ldp);
                float adij = A(ldp);
                *f += G(sr[j]) * mass[i] * mass[j] * adij * dpij / fmaxf(KIRI_EPSILON, ldp);
                float vj = mass[j] / density[j];
                *vS += vj * wij;
            }
            ++j;
    ...
    template <typename Func>
    __device__ void ComputeSFSandVoidage(
        float *voidN,
        float *voidD,
        const size_t i,
        size_t *label,
    ...
    __device__ void ComputeSFSandBuoyancyForces(
        float3 *f,
        float *vW,
        const size_t i,
        size_t *label,
    ...
    template <typename Func, typename GradientFunc>
    __device__ void ComputeSFWaterSeepageTerm(
        float3 *a,
        const size_t i,
        size_t *label,
        float3 *pos,
    ...
    template <typename Func>
    __device__ void ComputeSFSandAdhesionTerm(
        float3 *f,
        const size_t i,
        size_t *label,
        float3 *pos,
    ...
    __device__ void ComputeSFWaterPressureTerm(
        float3 *a,
        const size_t i,
        size_t *label,
        float3 *pos,
        ...
        GradientFunc nablaW)
    {
        while (j < cellEnd)
        {
            if (i != j && label[i] == label[j])
            {
                float densityi = density[i] / fmaxf(KIRI_EPSILON, voidage[i]);
                float densityj = density[j] / fmaxf(KIRI_EPSILON, voidage[j]);
                *a += -mass[j] * (pressure[i] / fmaxf(KIRI_EPSILON, densityi * densityi) + pressure[j] / fmaxf(KIRI_EPSILON, densityj * densityj)) * nablaW(pos[i] - pos[j]);
            }
            ++j;
        }
        ...
        while (j < cellEnd)
        {
            if (i != j && label[i] == label[j])
            {
                ...
                if (dot_dvdp < 0.f)
                {
                    float densityi = density[i] / fmaxf(KIRI_EPSILON, voidage[i]);
                    float densityj = density[j] / fmaxf(KIRI_EPSILON, voidage[j]);
                    float pij = -nu / (densityi + densityj) * (dot_dvdp / (lengthSquared(dpij) + KIRI_EPSILON));
    ...
    template <typename Func>
    __device__ void ComputeSFFluidDensity(
        float *density,
        const size_t i,
        size_t *label,
        float3 *pos,
    ...
    __global__ void ComputeSFDensity_CUDA(
        size_t *label,
        float3 *pos,
        float *mass,
        float *density,
...
#pragma unroll
        for (size_t m = 0; m < 27; ++m)
        {
            ...
            ComputeSFFluidDensity(&density[i], i, label, pos, mass, cellStart[hash_idx], cellStart[hash_idx + 1], W);
            ComputeBoundaryDensity(&density[i], pos[i], bPos, bVolume, rho0, bCellStart[hash_idx], bCellStart[hash_idx + 1], W);
        }

        return;
    }

    static __global__ void ComputeSFPressure_CUDA(
        size_t *label,
        float *density,
        float *pressure,
        const size_t num,
    ...
    template <typename Pos2GridXYZ, typename GridXYZ2GridHash, typename Func>
    __global__ void ComputeSFAvgFlow_CUDA(
        size_t *label,
        float3 *pos,
        float3 *vel,
        float *mass,
...
#pragma unroll
        for (size_t m = 0; m < 27; ++m)
        {
            ...
            ComputeSFSandVolume(&sand_volume, i, label, pos, mass, density, cellStart[hash_idx], cellStart[hash_idx + 1], W);
            ComputeSFAvgFlow(&avg_vel_s, &avg_vel_w, &v_s, &v_w, i, label, pos, vel, mass, density, cellStart[hash_idx], cellStart[hash_idx + 1], W);
        }
    ...
    template <typename Pos2GridXYZ, typename GridXYZ2GridHash, typename AttenuFunc, typename Func, typename AdhesionFunc>
...
#pragma unroll
        for (size_t m = 0; m < 27; ++m)
        {
            ...
            ComputeSFWaterAdhesionForces(&ad_forces, &v_s, i, label, pos, mass, density, saturation, cellStart[hash_idx], cellStart[hash_idx + 1], G, W, A);
        }
    ...
    template <typename Pos2GridXYZ, typename GridXYZ2GridHash, typename Func, typename AdhesionFunc>
    __global__ void ComputeMultiSFWaterAdhesionForces_CUDA(
        ...
        float3 *adhesionForce,
        float3 *avgAdhesionForce,
        const float3 *cda0asat,
        const float2 *amcamcp,
        const size_t num,
        size_t *cellStart,
        const int3 gridSize,
        Pos2GridXYZ p2xyz,
...
#pragma unroll
        for (size_t m = 0; m < 27; ++m)
        {
            ...
            ComputeSFWaterAdhesionForces(
                ...
                QuadraticBezierCoeff(0.f, 1.5f, 0.5f, 0.8f),
                W, A);
        }
...
#pragma unroll
        for (size_t m = 0; m < 27; ++m)
        {
            ...
            ComputeSFSandVoidage(&void_numerator, &void_denominator, i, label, pos, mass, density, voidage, cellStart[hash_idx], cellStart[hash_idx + 1], W);
        }
    ...
    template <typename Pos2GridXYZ, typename GridXYZ2GridHash, typename AttenuFunc, typename Func, typename GradientFunc>
...
#pragma unroll
        for (size_t m = 0; m < 27; ++m)
        {
            ...
            ComputeSFSandBuoyancyForces(&f, &v_w, i, label, pos, mass, density, pressure, voidage, cellStart[hash_idx], cellStart[hash_idx + 1], W, nablaW);
...
#pragma unroll
        ...
        if (voidage[i] < 1.f && voidage[i] > 0.f)
        {
            float dragCoeff = cda0asat[i].x * powf(voidage[i], 3.f) / powf(1.f - voidage[i], 2.f);
...
#pragma unroll
        for (size_t m = 0; m < 27; ++m)
        {
            ...
            ComputeSFWaterPressureTerm(&a, i, label, pos, mass, density, pressure, voidage, cellStart[hash_idx], cellStart[hash_idx + 1], nablaW);
            ...
            ComputeSFWaterSeepageTerm(&a, i, label, pos, mass, density, pressure, voidage, avgDragForce, cellStart[hash_idx], cellStart[hash_idx + 1], W, nablaW);

            ComputeBoundaryPressure(&a, pos[i], rho0, pressure[i], bPos, bVolume, rho0, bCellStart[hash_idx], bCellStart[hash_idx + 1], nablaW);
    ...
    static __global__ void ComputeSFWetSandColor_CUDA(
        size_t *label,
        float *saturation,
        float *maxSaturation,
        float3 *col,
