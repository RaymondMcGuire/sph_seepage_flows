 ...
 * @Date: 2023-07-01 22:35:58
 * @LastEditors: Xu.WANG raymondmgwx@gmail.com
 * @LastEditTime: 2023-07-24 10:39:09
...
void Seepage_Uni_Slide_WCSPH()
{
    ...
    // auto cuda_lowest_point = make_float3(-0.75f, -0.005f, -0.005f);
    // //左下角与右上角 坐标顺序 x y z auto cuda_highest_point = make_float3(8.85f,
    // 0.5f, 0.105f);-1.375f
    auto cuda_lowest_point =
        make_float3(-30.f, -0.2f, -0.2f); // 左下角与右上角 坐标顺序 x y z
    ...
    CUDA_SEEPAGEFLOW_PARAMS.dem_tan_friction_angle =
        0.537f; // 大洼水库内摩擦角为28.25度0.537
    CUDA_SEEPAGEFLOW_PARAMS.dem_damping = 0.5f;

    // 拟合参数 毛细管力参数
    // c0属于[0.6,0.9];csat属于[0,0.1];cmc最优含水率属于[0.8,1];
    // cd为渗透系数cm/s，替KozenyCarman常数;cmc_p属于[,]bizer曲线系数
    CUDA_SEEPAGEFLOW_PARAMS.sf_c0 = 0.8f;
    CUDA_SEEPAGEFLOW_PARAMS.sf_cd =
        0.5f; // 渗透系数建立与kozeny carman常数公式关系 拟合一下 单位：cm/s  0.014
    CUDA_SEEPAGEFLOW_PARAMS.sf_csat = 0.1f;
    CUDA_SEEPAGEFLOW_PARAMS.sf_cmc = 0.9f;
    CUDA_SEEPAGEFLOW_PARAMS.sf_cmc_p = 0.9; // 最优时刻含水量

    // 拟合参数求附着力ahensioon 粘附系数 a0初始;
    // asat饱和情况粘附系数;amc最优;amc_p属于bizer曲线系数
    ...
    CUDA_SEEPAGEFLOW_PARAMS.sf_dry_sand_color =
        make_float3(0.55f, 0.79552f, 0.5984f);
    ...
    // auto boundary_particles_data =
    // VTKReader::ReadPoints(boundary_vtk_file_path);
    ...
    // material type (SF: unified material; MULTI_SF: multiple types of
    // materials)材料类型：统一粒径；多粒径
    ...
    //     volumeEmitter->BuildSeepageflowShapeMultiVolume(multi_volume_data,
    //     sand_shape, 0.09f,
    //         CUDA_SEEPAGEFLOW_PARAMS.sf_dry_sand_color,CUDA_SEEPAGEFLOW_PARAMS.dem_density,
    //         cda0asat, amcamcp, false, offset2Ground,
    ...
    // 读取bgeo的物体案例(bgeo/bunny/bunny.bgeo)
    auto sand_shape = ReadBgeoFileForGPU("bunny", "bunny");
    volumeEmitter->BuildSeepageflowShapeMultiVolume(
        multi_volume_data, sand_shape, CUDA_SEEPAGEFLOW_PARAMS.sf_dry_sand_color,
        CUDA_SEEPAGEFLOW_PARAMS.dem_density, cd_a0_asat[0], amc_amcp[0], offset2Ground,
        CUDA_BOUNDARY_PARAMS.lowest_point.y, make_float2(0.f, 0.f));
...
void WCSPH_Bunny_Example()
{
    KIRI_LOG_DEBUG("Seepageflow: WCSPH_Bunny_Example");
    ...
    // sph params
    CUDA_SEEPAGEFLOW_PARAMS.sph_density = 1000.f;
    CUDA_SEEPAGEFLOW_PARAMS.sph_particle_radius = 0.0045f; // 0.01f;
    ...
    SeepageflowMultiVolumeData multi_volume_data;
    multi_volume_data.min_radius = KIRI::Huge<size_t>();
    auto volumeEmitter = std::make_shared<CudaVolumeEmitter>();

    // bgeo文件所在的文件夹
    std::vector<std::string> shape_folders;

    // bgeo文件的名称以及类型，0为水1为沙子
    std::vector<VolumeFilesType> shape_files;

    // 测试物体1: bunny/bunny.bgeo 将读取到的粒子设置为水
    // 注意：bgeo文件中采样得到的水粒子半径应与设置的sph粒子半径保持一致
    shape_folders.emplace_back("bunny");
    shape_files.emplace_back(VolumeFilesType("bunny", 0));

    // object 2: dam/dam2.bego
    // shape_folders.emplace_back("dam");
    // shape_files.emplace_back("dam2");

    // 对不同材料物体设置不同渗透系数以及附着力系数
    // 注意：如果是水的bgeo文件，这个参数请随意设置，并不会生效
    std::vector<float3> cd_a0_asat;
    std::vector<float2> amc_amcp;
    ...
    for (auto i = 0; i < shape_folders.size(); i++)
    {
        auto cda0asat = cd_a0_asat[i];
        auto amcamcp = amc_amcp[i];
        auto volume_data = ReadBgeoFileForGPU(shape_folders[i], shape_files[i].name);

        if (shape_files[i].type == 0)
            volumeEmitter->BuildWaterVolumeData(
                multi_volume_data,
                volume_data,
                CUDA_SPH_EMITTER_PARAMS.emit_col,
                CUDA_SEEPAGEFLOW_PARAMS.sph_mass,
                CUDA_SEEPAGEFLOW_PARAMS.sph_particle_radius);
        else if (shape_files[i].type == 1)
            volumeEmitter->BuildSandVolumeData(
                multi_volume_data,
                volume_data,
                CUDA_SEEPAGEFLOW_PARAMS.sf_dry_sand_color,
                CUDA_SEEPAGEFLOW_PARAMS.dem_density,
    ...
    KIRI_LOG_INFO("Number of total particles = {0}", multi_volume_data.pos.size());
...
void main()
{
    ...
    while (CUDA_SEEPAGEFLOW_APP_PARAMS.run)
    {
            ...
            SimCount < TotalFrameNumber + RunLiquidNumber)
        {
            ...
            //    CUDA_SPH_EMITTER_PARAMS.run = false;
            //}
            // if (SimCount == RestartLiquidNumber)   //重新开始流量时间设置
            // 中途可设置放水边界
            //{
            //    CUDA_SPH_EMITTER_PARAMS.run = true;
            //}
            ...
            if (CUDA_SEEPAGEFLOW_APP_PARAMS.enable_write2file)
            {
                ...
                // auto cpu_acc =TransferGPUData2CPU(particles->GetAccPtr(),
                // particles->Size()); auto cpu_sat =
                // TransferGPUData2CPU(particles->GetSaturationPtr(), particles->Size());
                auto cpu_sat = TransferGPUData2CPU(particles->GetMaxSaturationPtr(),
                                                   particles->Size());

                // auto cpu_rho =
                //    TransferGPUData2CPU(particles->GetDensityPtr(),
                //    particles->Size());
