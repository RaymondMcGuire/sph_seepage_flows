 ...
 * @LastEditTime: 2023-05-16 22:19:54
...
#include <vtk_helper/vtk_structure_grid.h>
#include <filesystem>
// clang-format on
...
void Seepage_UniBunny_WCSPH() {
  ...
  for (auto i = 0; i < shape_folders.size(); i++) {
    ...
    auto sand_shape = ReadBgeoFileForGPU(shape_folders[i], shape_files[i]);
...
void Seepage_MSBunny_WCSPH() {

  KIRI_LOG_DEBUG("Seepagei_MS_Bunny_WCSPH: SetupParams");
  ExampleName = "seepageflow_ms_bunny_wcsph";
  // export path
  strcpy(CUDA_SEEPAGEFLOW_APP_PARAMS.bgeo_export_folder,
         (String(EXPORT_PATH) + "bgeo/" + ExampleName).c_str());
  ...
  auto cuda_world_size = cuda_highest_point - cuda_lowest_point;
  auto cuda_world_center = (cuda_highest_point + cuda_lowest_point) / 2.f;
  CUDA_SEEPAGEFLOW_APP_PARAMS.max_num = 400000;
  ...
  CUDA_SEEPAGEFLOW_PARAMS.sf_c0 = 3.5f;
  CUDA_SEEPAGEFLOW_PARAMS.sf_cd = 0.5f;
  CUDA_SEEPAGEFLOW_PARAMS.sf_csat = 0.f;
  CUDA_SEEPAGEFLOW_PARAMS.sf_cmc = 3.7f;
  CUDA_SEEPAGEFLOW_PARAMS.sf_cmc_p = 0.01f;
...
void Seepage_MSDam_WCSPH() {

  KIRI_LOG_DEBUG("Example: Seepage_MSDam_WCSPH");
  ExampleName = "seepageflow_ms_dam_wcsph_cd0.015";
  // export path
  strcpy(CUDA_SEEPAGEFLOW_APP_PARAMS.bgeo_export_folder,
         (String(EXPORT_PATH) + "bgeo/" + ExampleName).c_str());
...
void Seepage_MSDam_OverTop_DFSPH() {

  KIRI_LOG_DEBUG("Example:  Seepage_MSdamovertop_DFSPH");
  ExampleName = "seepageflow_ms_damovertop_dfsph";
  // export path
  strcpy(CUDA_SEEPAGEFLOW_APP_PARAMS.bgeo_export_folder,
         (String(EXPORT_PATH) + "bgeo/" + ExampleName).c_str());

  // scene config
  auto cuda_lowest_point = make_float3(-0.62f, -1.0f, -30.2f);//左下角与右上角 坐标顺序 x y z
  auto cuda_highest_point = make_float3(1.42f, 1.8f, 4.8f);//-0.63
  auto cuda_world_size = cuda_highest_point - cuda_lowest_point;
  auto cuda_world_center = (cuda_highest_point + cuda_lowest_point) / 2.f;
...
//   boundaryEmitter->BuildBoundaryShapeVolume(boundaryData,boudnary_shapes_data);
  ...
  // object 1: bunny/bunny.bego
  shape_folders.emplace_back("damovertop");
  shape_files.emplace_back("damovertop");
...
void main() {
  KiriLog::Init();
    ...
    PolygonalMeshData data(dimX, dimY, dimZ, spacing);

    std::vector<vtkIdType>pointIds;
    // 添加标量数据
    std::vector<double> scalarData;
    for (int i = 0; i < dimX * dimY * dimZ; ++i) {
        scalarData.push_back(i * 0.1);
        pointIds.push_back(i);
    }
    data.AddScalarData("ScalarData", scalarData);

    // 添加多边形信息
    
data.AddPolygon(pointIds);

data.WriteToFile("output123.vtk");
