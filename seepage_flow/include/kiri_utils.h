...
Vec_Float4 ReadBgeoFileForGPU(String Folder, String Name,
                              bool flip_yz = false) {
...
Vec_Float4 ReadMultiBgeoFilesForGPU(Vec_String folders, Vec_String file_names,
                                    bool flip_yz = false) {
  ...
  for (auto n = 0; n < folders.size(); n++) {
    auto file_path = std::string(DB_PBR_PATH) + root_folder + "/" + folders[n] +
                     "/" + file_names[n] + extension;
    Partio::ParticlesDataMutable *data = Partio::read(file_path.c_str());
    ...
    KIRI_LOG_INFO("Loaded Bgeo File={0}, Number of Particles={1}",
                  file_names[n], data->numParticles());
...
void ExportBgeoFileCUDA(const std::string &folder_path,
                        const std::string &file_name, const float3 *positions,
                        const float3 *colors, const float *radius,
                        const size_t *labels, const size_t particles_num) {
  ...
  try {
    ...
    Partio::ParticleAttribute position_attr =
        p->addAttribute("position", Partio::VECTOR, 3);
    Partio::ParticleAttribute color_attr =
        p->addAttribute("Cd", Partio::FLOAT, 3);
    Partio::ParticleAttribute pscale_attr =
        p->addAttribute("pscale", Partio::FLOAT, 1);
    Partio::ParticleAttribute label_attr =
