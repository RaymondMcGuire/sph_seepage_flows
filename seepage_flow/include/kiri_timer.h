...
class KiriTimeStep {
public:
  KiriTimeStep(float time = 0.0f) : mTime(time) {}
...
class KiriTimer {
public:
  KiriTimer() : mName("Default") { Restart(); }

  explicit KiriTimer(const String &name) : mName(name) { Restart(); }

  inline void Restart() { mStartTime = std::chrono::steady_clock::now(); }
  ...
  /***
   * @description:
   * @param {reset:reset timer or not; unitMs: print ms / sec; tip: print extra
   * info; kill: after print timer, kill thread or not}
   * @return {void}
   */
  void Log(bool reset = false, const String &tip = "", bool unitMs = true,
           bool kill = false) {
    if (unitMs) {
      if (tip.length() > 0)
        KIRI_LOG_INFO("KiriTimer({0}) Time elapsed:{1} ms", tip,
                      Elapsed() * 1000.f);
      else
        KIRI_LOG_INFO("KiriTimer({0}) Time elapsed:{1} ms", mName,
                      Elapsed() * 1000.f);
    } else {
      if (tip.length() > 0)
