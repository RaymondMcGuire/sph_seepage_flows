 ...
 * @LastEditors: Xu.WANG
 * @Description: 
 * @FilePath: \Kiri\KiriPBSCuda\src\kiri_pbs_cuda\solver\seepage_flow\cuda_wcsph_sf_solver.cu
 */
...
namespace KIRI
{
  ...
  void CudaWCSphSFSolver::ComputePressure(
  ...
  void CudaWCSphSFSolver::ComputeSubTimeStepsByCFL(
      CudaSFParticlesPtr &particles,
      const float sphMass,
      const float dt,
      ...
      float renderInterval)
  {
    ...
    float3 maxAcc = *(thrust::max_element(
        accArray,
        accArray + particles->Size(),
        ThrustHelper::CompareLengthCuda<float3>()));

    float maxForceMagnitude = length(maxAcc) * sphMass;
    float timeStepLimitBySpeed = mTimeStepLimitBySpeedFactor * kernelRadius / mSpeedOfSound;
    float timeStepLimitByForce = mTimeStepLimitByForceFactor * std::sqrt(kernelRadius * sphMass / maxForceMagnitude);
    float desiredTimeStep = std::min(mTimeStepLimitScale * std::min(timeStepLimitBySpeed, timeStepLimitByForce), dt);
