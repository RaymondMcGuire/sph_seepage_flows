 ...
 * @FilePath:
 * \sph_seepage_flows\seepage_flows_cuda\src\kiri_pbs_cuda\solver\seepage_flow\cuda_sph_sf_solver.cu
 * @Description:
...
void CudaSphSFSolver::ComputeDensity(
  ...
  ComputeSFDensity_CUDA<<<mCudaGridSize, KIRI_CUBLOCKSIZE>>>(
      ...
      particles->GetDensityPtr(), rho0, rho1, particles->Size(),
...
void CudaSphSFSolver::ComputePressure(CudaSFParticlesPtr &particles,
  ...
  ComputeSFPressure_CUDA<<<mCudaGridSize, KIRI_CUBLOCKSIZE>>>(
      ...
      particles->GetPressurePtr(), particles->Size(), rho0, stiff, 1.f);
  KIRI_CUCALL(cudaDeviceSynchronize());
  KIRI_CUKERNAL();
}
...
void CudaSphSFSolver::ComputeAvgFlowVelocity(CudaSFParticlesPtr &particles,
  ...
  ComputeSFAvgFlow_CUDA<<<mCudaGridSize, KIRI_CUBLOCKSIZE>>>(
      ...
      particles->GetSaturationPtr(), particles->Size(), cellStart.Data(),
      gridSize,
      ThrustHelper::Pos2GridXYZ<float3>(lowestPoint, kernelRadius, gridSize),
      ThrustHelper::GridXYZ2GridHash(gridSize), Poly6Kernel(kernelRadius));
  KIRI_CUCALL(cudaDeviceSynchronize());
...
void CudaSphSFSolver::ComputeSFSandVoidage(CudaSFParticlesPtr &particles,
  ...
  ComputeSFSandVoidage_CUDA<<<mCudaGridSize, KIRI_CUBLOCKSIZE>>>(
      particles->GetLabelPtr(), particles->GetPosPtr(), particles->GetMassPtr(),
      particles->GetDensityPtr(), particles->GetVoidagePtr(), particles->Size(),
...
void CudaSphSFSolver::ComputeSFSandLinearMomentum(
  ...
  ComputeSFSandLinearMomentum_CUDA<<<mCudaGridSize, KIRI_CUBLOCKSIZE>>>(
      particles->GetLabelPtr(), particles->GetPosPtr(), particles->GetVelPtr(),
      particles->GetAccPtr(), particles->GetMassPtr(),
      particles->GetDensityPtr(), particles->GetPressurePtr(),
      particles->GetVoidagePtr(), particles->GetSaturationPtr(),
      particles->GetAvgFlowVelPtr(), particles->GetAvgDragForcePtr(),
