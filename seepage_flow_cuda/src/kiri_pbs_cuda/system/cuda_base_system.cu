 ...
 * @Description:
 * @FilePath:
 * \sph_seepage_flows\seepage_flows_cuda\src\kiri_pbs_cuda\system\cuda_base_system.cu
 */

#include <kiri_pbs_cuda/system/cuda_base_system.cuh>

namespace KIRI {
CudaBaseSystem::CudaBaseSystem(CudaBoundaryParticlesPtr &boundaryParticles,
    ...
    : mBoundaries(std::move(boundaryParticles)), mSolver(std::move(solver)),
