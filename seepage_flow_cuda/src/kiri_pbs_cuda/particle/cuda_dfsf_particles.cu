...
struct AdvectVelForDFFluid {
  float mDt;
  __host__ __device__ AdvectVelForDFFluid(const float dt)
      : mDt(dt) {}
...
struct AdvectVelForDFSand {
  float mDt;
  __host__ __device__ AdvectVelForDFSand(const float dt)
      : mDt(dt) {}
...
void CudaDFSFParticles::AdvectFluidVel(const float dt) {
  ...
  thrust::transform(thrust::device, waterAccDataIterator, waterAccDataIterator + Size(),
                    mVel.Data(), AdvectVelForDFFluid(dt));
...
// sand vel advect
  ...
  thrust::transform(thrust::device, sandVelDataIterator, sandVelDataIterator + Size(),
                    mVel.Data(), AdvectVelForDFSand(dt));
