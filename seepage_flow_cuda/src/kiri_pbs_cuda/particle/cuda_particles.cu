 ...
 * @Date: 2023-05-17 15:35:47
 * @LastEditors: Xu.WANG raymondmgwx@gmail.com
 * @LastEditTime: 2023-05-17 15:35:47
...
namespace KIRI {
   ...
   CudaParticles::CudaParticles(const Vec_Float3 &p)
      : mPos(p.size()),mId(p.size()), mParticle2Cell(p.size()), mNumOfParticles(p.size()),
        mNumOfMaxParticles(p.size()) {
      thrust::device_ptr<int> id_ptr(mId.Data());
      thrust::device_ptr<int> id_end_ptr(mId.Data()+p.size());
     thrust::sequence(id_ptr,id_end_ptr, 1);
    KIRI_CUCALL(cudaMemcpy(mPos.Data(), &p[0], sizeof(float3) * p.size(),
                           cudaMemcpyHostToDevice));
  }

   CudaParticles::CudaParticles(const size_t numOfMaxParticles, const Vec_Float3 &p)
