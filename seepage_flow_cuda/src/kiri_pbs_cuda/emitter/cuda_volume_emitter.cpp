 ...
 * @Date: 2023-07-01 22:57:05
 * @LastEditors: Xu.WANG raymondmgwx@gmail.com
 * @LastEditTime: 2023-07-24 10:40:46
...
namespace KIRI
{
  ...
  void CudaVolumeEmitter::BuildSandVolumeData(
      SeepageflowMultiVolumeData &data, Vec_Float4 shape, float3 color,
      float sandDensity, float3 cda0asat, float2 amcamcp, bool offsetY,
      float worldLowestY, float2 offsetXZ)
  {
  ...
  void CudaVolumeEmitter::BuildWaterVolumeData(
      SeepageflowMultiVolumeData &data, Vec_Float4 shape, float3 color,
      float mass, float radius)
  {
    ...
    data.min_radius = std::min(data.min_radius, radius);

    for (size_t i = 0; i < shape.size(); i++)
    {
      data.pos.emplace_back(make_float3(shape[i].x, shape[i].y,
                                        shape[i].z));
      data.col.emplace_back(color);
