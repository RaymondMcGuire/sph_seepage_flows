 ...
 * @FilePath:
 * \sph_seepage_flows\seepage_flow_cuda\include\kiri_pbs_cuda\particle\cuda_sf_particles.cuh
 * @Description:
...
public:
  explicit CudaSFParticles::CudaSFParticles(const size_t numOfMaxParticles)
      : CudaParticles(numOfMaxParticles), mLabel(numOfMaxParticles),
        ...
        mAngularVel(numOfMaxParticles), mAngularAcc(numOfMaxParticles),
        mInertia(numOfMaxParticles), mPressure(numOfMaxParticles),
        mDensity(numOfMaxParticles), mMass(numOfMaxParticles),
        mVoidage(numOfMaxParticles), mSaturation(numOfMaxParticles),
        mMaxSaturation(numOfMaxParticles), mAvgFlowVel(numOfMaxParticles),
        mAvgDragForce(numOfMaxParticles), mAdhesionForce(numOfMaxParticles),
        mAvgAdhesionForce(numOfMaxParticles), mCdA0Asat(numOfMaxParticles),
        mAmcAmcp(numOfMaxParticles) {}

  explicit CudaSFParticles::CudaSFParticles(
      const Vec_Float3 &p, const Vec_Float3 &col, const Vec_SizeT &label,
      const Vec_Float &mass, const Vec_Float &inertia, const Vec_Float &radius)
      : CudaParticles(p), mLabel(p.size()), mRadius(p.size()), mVel(p.size()),
        mAcc(p.size()), mAngularVel(p.size()), mAngularAcc(p.size()),
        mCol(p.size()), mPressure(p.size()), mDensity(p.size()),
        mMass(p.size()), mInertia(p.size()), mVoidage(p.size()),
        mSaturation(p.size()), mMaxSaturation(p.size()), mAvgFlowVel(p.size()),
  ...
  explicit CudaSFParticles::CudaSFParticles(
      ...
      const Vec_Float &inertia, const Vec_Float &radius)
  ...
  explicit CudaSFParticles::CudaSFParticles(
      ...
      const Vec_Float3 &col, const Vec_SizeT &label, const Vec_Float &mass,
      const Vec_Float &inertia, const Vec_Float &radius,
      const Vec_Float3 &cda0asat, const Vec_Float2 &amcamcp)
      : CudaParticles(numOfMaxParticles, p), mLabel(numOfMaxParticles),
        mRadius(numOfMaxParticles), mVel(numOfMaxParticles),
        mAcc(numOfMaxParticles), mCol(numOfMaxParticles),
        mAngularVel(numOfMaxParticles), mAngularAcc(numOfMaxParticles),
