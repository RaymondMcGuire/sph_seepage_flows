 ...
 * @Description:
 * @FilePath:
 * \Kiri\KiriPBSCuda\include\kiri_pbs_cuda\system\cuda_sph_system_gpu.cuh
 */
...
__device__ void ComputeBoundaryVolume(float *delta, const size_t i, float3 *pos,
...
__global__ void ComputeBoundaryVolume_CUDA(float3 *pos, float *volume,
                                           const size_t num, size_t *cellStart,
                                           const int3 gridSize,
                                           Pos2GridXYZ p2xyz,
...
#pragma unroll
  for (int m = 0; m < 27; ++m) {
    ...
    ComputeBoundaryVolume(&volume[i], i, pos, cellStart[hash_idx],
